...
conn_create_fd <- function(fd, encoding = "", close = TRUE) {
  assert_that(
    ...
    is_flag(close))
  fd <- as.integer(fd)
  entrace_call(c_processx_connection_create_fd, fd, encoding, close)
}
...
conn_create_pipepair <- function(encoding = "",
  ...
  entrace_call(c_processx_connection_create_pipepair, encoding, nonblocking)
}
...
processx_conn_read_chars <- function(con, n = -1) {
  ...
  entrace_call(c_processx_connection_read_chars, con, n)
}
...
processx_conn_read_lines <- function(con, n = -1) {
  assert_that(is_connection(con), is_integerish_scalar(n))
  entrace_call(c_processx_connection_read_lines, con, n)
}
...
processx_conn_is_incomplete <- function(con) {
  ...
  ! entrace_call(c_processx_connection_is_eof, con)
}
...
processx_conn_write <- function(con, str, sep = "\n", encoding = "") {
  ...
  invisible(entrace_call(c_processx_connection_write_bytes, con, str))
}
...
conn_create_file <- function(filename, read = NULL, write = NULL) {
  ...
  entrace_call(c_processx_connection_create_file, filename, read, write)
}
...
conn_set_stdout <- function(con, drop = TRUE) {
  ...
  flush(stdout())
  invisible(entrace_call(c_processx_connection_set_stdout, con, drop))
}
...
conn_set_stderr <- function(con, drop = TRUE) {
  ...
  flush(stderr())
  invisible(entrace_call(c_processx_connection_set_stderr, con, drop))
}
...
conn_get_fileno <- function(con) {
  entrace_call(c_processx_connection_get_fileno, con)
}
...
conn_disable_inheritance <- function() {
  entrace_call(c_processx_connection_disable_inheritance)
}
...
processx_conn_close <- function(con, ...) {
  entrace_call(c_processx_connection_close, con)
}
...
is_valid_fd <- function(fd) {
  ...
  fd <- as.integer(fd)
  entrace_call(c_processx_is_valid_fd, fd)
}
