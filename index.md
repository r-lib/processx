...
The [`run()`](http://processx.r-lib.org/reference/run.md) function runs
an external command. It requires a single command, and a character
...
By default [`run()`](http://processx.r-lib.org/reference/run.md) throws
an error if the process exits with a non-zero status code. To avoid
this, specify `error_on_status = FALSE`:
...
Note that [`run()`](http://processx.r-lib.org/reference/run.md) is
...
[`run()`](http://processx.r-lib.org/reference/run.md) puts a calming
...
[`run()`](http://processx.r-lib.org/reference/run.md) can call an R
...
[`poll()`](http://processx.r-lib.org/reference/poll.md) function that
