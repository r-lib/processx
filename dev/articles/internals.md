...
[`conn_connect_fifo()`](http://processx.r-lib.org/dev/reference/processx_fifos.md)
...
[`poll()`](http://processx.r-lib.org/dev/reference/poll.md) to see if
there is data in the FIFO. There is no way to know currently when the
other end connects, but once it writes data,
[`poll()`](http://processx.r-lib.org/dev/reference/poll.md) will return
`ready` .
...
- `conn_read_*()` will fail on Windows if the other end or the FIFO is
  ...
  [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) first and
  only read if it returns `ready`.
  [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) will not
...
- A [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) on a
  FIFO can return `ready` and then have no data on the next
  `conn_read_*()`, at least on Windows. This is because at the first
  [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) (on
  Windows) we need to start an asynchronous operation for waiting for
  the other end of the pipe to connect. Once this returns,
  [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) will
  signal `ready`, but there might be no data to read just yet.

### Known issues

- A [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) on a
  FIFO might not return if the writer on the other end (all the writers)
  close the FIFO. The
  [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) will only
...
- Non-ASCII file names or pipe name are currently an issue. (Just like
  ...
  [`conn_create_file()`](http://processx.r-lib.org/dev/reference/processx_connections.md)
...
- [`conn_connect_unix_socket()`](http://processx.r-lib.org/dev/reference/processx_sockets.md)
...
- You can use
  [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) to wait
  for a client to connect to the server. If a client is available,
  [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) returns
  `"connect"` .

- Once a client is available, call
  [`conn_accept_unix_socket()`](http://processx.r-lib.org/dev/reference/processx_sockets.md)
  to accept it.
...
- [`poll()`](http://processx.r-lib.org/dev/reference/poll.md) only uses
  the read-side of the connections, both for the server and the client.
...
- [`conn_unix_socket_state()`](http://processx.r-lib.org/dev/reference/processx_sockets.md)
...
[`poll()`](http://processx.r-lib.org/dev/reference/poll.md) for a client
...
This is as usual.
[`poll()`](http://processx.r-lib.org/dev/reference/poll.md) will return
with `ready` if the other end closes the connection. You can call
[`conn_is_incomplete()`](http://processx.r-lib.org/dev/reference/processx_connections.md)
