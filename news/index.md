...
- New experimental `pipeline` R6 class for running two or more processes
  ...
  without passing through R. Works on Unix and Windows
  ([\#280](https://github.com/r-lib/processx/issues/280)).
...
- New `linux_pdeathsig` argument to `process$new()`: on Linux, the child
  ...
  (e.g. [`tools::SIGKILL`](https://rdrr.io/r/tools/pskill.html)).
  Ignored on non-Linux platforms
  ([\#36](https://github.com/r-lib/processx/issues/36)).

- New `process$get_end_time()` method returns the time when the process
  exited as a `POSIXct`, or `NULL` if it is still running
  ([\#218](https://github.com/r-lib/processx/issues/218)).

- `process$new()` and
  [`run()`](http://processx.r-lib.org/reference/run.md) now support
...
- [`run()`](http://processx.r-lib.org/reference/run.md) now supports
...
- [`run()`](http://processx.r-lib.org/reference/run.md) and
  `process$new()` now support `encoding = "binary"` to capture binary
  output. In this mode
  [`run()`](http://processx.r-lib.org/reference/run.md) returns `stdout`
  and `stderr` as raw vectors, and `process$read_output()` /
...
- On Linux, `process$get_start_time()` now returns the correct
  wall-clock start time. Previously it was systematically ~0.3–0.5 s too
  early because the boot time was read from `/proc/stat btime`, which is
  truncated to whole seconds. processx now derives the boot time from
  `CLOCK_REALTIME − CLOCK_MONOTONIC`, which has nanosecond precision.
  The ps package is updated in tandem to accept handles created by
  either the old or the new method, so new ps + old processx continues
  to work ([\#394](https://github.com/r-lib/processx/issues/394),
  [\#402](https://github.com/r-lib/processx/issues/402)).
...
- [`run()`](http://processx.r-lib.org/reference/run.md) now sets
  ...
  [`run()`](http://processx.r-lib.org/reference/run.md) with a
...
- [`run()`](http://processx.r-lib.org/reference/run.md) now does not
...
- You can now append environment variables to the ones set in the
  ...
  [`run()`](http://processx.r-lib.org/reference/run.md) and for
...
- [`run()`](http://processx.r-lib.org/reference/run.md) is now much
  faster with large standard output or standard error
  ([\#286](https://github.com/r-lib/processx/issues/286)).

- [`run()`](http://processx.r-lib.org/reference/run.md) can now discard
  the standard output and error or redirect them to file(s), instead of
  collecting them.
...
- [`run()`](http://processx.r-lib.org/reference/run.md) now does a
...
- Now [`run()`](http://processx.r-lib.org/reference/run.md) does not
...
- [`run()`](http://processx.r-lib.org/reference/run.md) now prints the
  last 10 lines of the standard error stream on error, if
  `echo = FALSE`, and it also prints the exit status of the process.

- [`run()`](http://processx.r-lib.org/reference/run.md) now includes the
  standard error in the condition signalled on interrupt.
...
- [`run()`](http://processx.r-lib.org/reference/run.md) now passes extra
...
- [`run()`](http://processx.r-lib.org/reference/run.md) can now redirect
  the standard error to the standard output, via the new
...
- [`run()`](http://processx.r-lib.org/reference/run.md) now always kills
  the process (and its process tree if `cleanup_tree` is `TRUE`) before
  exiting. This also closes all pipe connections
...
- [`poll()`](http://processx.r-lib.org/reference/poll.md) now works with
  connections objects, and they can be mixed with process objects
...
- When [`run()`](http://processx.r-lib.org/reference/run.md) is
...
- [`run()`](http://processx.r-lib.org/reference/run.md): Better message
  on interruption, kill process when interrupted.
